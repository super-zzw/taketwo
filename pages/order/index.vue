	...
	<view class="container" >
		...
		<block v-if="orderList.length>0&&loaded">
					...
					<view class="order-head">
						...
						<view :class="!(item.status==1||item.status==2)?'right between':'right'">
							...
							<block v-if="currentRole==2">
								<text class="status" v-if="item.status==1">待接单</text>
								<text class="status" v-if="item.status==2">待配送</text>
			...
			<view class="bottom" v-if="index==1||index==2">
				...
				<view class="right" v-if="index==1" @tap="receiving">接单</view>
				<view class="right" v-if="index==2&&currentRole==1" @tap="toSort">分拣</view>
				<view class="right" v-if="index==2&&currentRole==2" @tap="deliveryFinish">配送完成</view>
			</view>
		</block>
			...
			<view class="cu-dialog dialog dialog1">
				...
				<view class="content">
					...
					<view class="tt">
						...
						<view class="cont">
							<view class="cont1">
								<view  v-for="(item,i) in config.logisticsList" :key="item.id" :class="wuliuIndex==i?'box active':'box'" @tap="wuliuIndex=i,searchData.logisticsId=item.id">
					...
					<view class="tt">
						...
						<view class="cont">
							<view class="cont1">
								<view  :class="orderIndex==2?'box active':'box'" @tap="orderIndex=2,searchData.type=2">
									加急
								</view>
								<view  :class="orderIndex==1?'box active':'box'" @tap="orderIndex=1,searchData.type=1">
									定时送
								</view>
		...
		<view class="cu-modal bottom-modal" :class="modalName=='fenjian'?'show':''">
			<view class="cu-dialog dialog dialog2">
				...
				<view class="tagsList">
					<view v-for="(item,i) in labelList" :key="i" :class="tagIndex==i?'tag active':'tag'" @click="tagIndex=i,searchData.labelId=item.id">
						{{item.label}}
					</view>
				</view>
			...
			<view class="cu-dialog dialog dialog3">
				...
				<view class="mdBox">
					...
					<view class="mdCol mdCol2">
						...
						<view class="mdCol1">
							<view class="mdItem" :class="selectAddr[1]==i2?'mdItem active':'mdItem'" v-for="(item2,i2) in addressList[selectAddr[0]].layers_num"
							 :key="i2" @click="change2(i2)">
								{{item2+1}}层
							</view>
 ...
 console.log(this.memberInfo)
		...
		methods: {
			...
			addressConfirm() {
				...
				this.searchData.floor=(this.selectAddr[1] + 1) + '层'
				// this.searchData.block=this.addressList[this.selectAddr[0]].building_name
			...
			receiving() {
				...
				if (this.currentRole == 1) {
					...
					data = {
						sorterReceivingPOJO: JSON.stringify(this.selectDataList)
					}
				} else {
					url = '/legwork/team/member/delivery/receiving/'
					data = {
						deliveryReceivingPOJO: JSON.stringify(this.selectDataList)
					}
				}

				this.$http({
					url: url + this.memberInfo.user_info.tid,
			...
			deliveryFinish() {
				...
				this.$http({
					...
					method: 'POST',
					data: {
						ids:JSON.stringify(this.selectDataList) 
					}
				}).then(res => {
					uni.showToast({
						title: '配送成功',
...
this.initSearchBox()
			...
			select(val, index, id) {
				...
				if (val == -1) {
					...
					} else if (this.currentRole == 2 && this.index == 2) {
						data = this.orderList[index].deliveryId
					}
